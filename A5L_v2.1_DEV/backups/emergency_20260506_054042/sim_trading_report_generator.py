...
⚠️ 数据访问规范：
- 所有数据必须通过 unified_position_manager 读取
- 禁止直接访问 JSON 文件
- 这是架构整改后的标准做法
"""
...
class SimTradingReportGenerator:
    ...
    def load_a_share_data(self) -> Dict:
        """加载A股模拟交易数据 - 使用统一数据接口"""
        try:
            data = self.position_manager.get_positions("CN")
            return {
                "market": "A股",
                "currency": "CNY",
                "portfolio": {
            ...
            print("⚠️ A股数据文件不存在，返回空数据")
            return {
                "market": "A股",
                "currency": "CNY",
                "portfolio": {"cash": 5000000.0, "positions": {}, "total_value": 5000000.0},
                "trades": [],
    ...
    def load_hk_data(self) -> Dict:
        """加载港股模拟交易数据 - 使用统一数据接口"""
        try:
            data = self.position_manager.get_positions("HK")
            return {
                "market": "港股",
                "currency": "HKD",
                "portfolio": {
    ...
    def load_us_data(self) -> Dict:
        """加载美股模拟交易数据 - 使用统一数据接口"""
        try:
            data = self.position_manager.get_positions("US")
            return {
                "market": "美股",
                "currency": "USD",
                "portfolio": {
    ...
    def calculate_metrics(self, data: Dict) -> Dict:
        ...
        # 使用market_value字段或计算市值 (quantity * current_price)
        total_market_value = sum(
            pos.get("market_value", 0) or 
            pos.get("quantity", 0) * pos.get("current_price", pos.get("cost_basis", 0))
...
"""
        ...
        if positions:
            ...
            for pos in positions:
                ...
                # 支持两种成本字段名: avg_cost (旧) 或 cost_basis (新)
                cost = pos.get("avg_cost") or pos.get("cost_basis", 0)
                current_price = pos.get("current_price", cost)
                mv = pos.get("market_value", qty * current_price)
