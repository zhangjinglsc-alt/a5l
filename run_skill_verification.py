...
# 第一步：读取昨天的L3预测
try:
    with open(f"data/review_layers/layer3_comprehensive_{yesterday}.json", "r", encoding="utf-8") as f:
        yesterday_report = json.load(f)
    predictions = yesterday_report.get("predictions", {})
except FileNotFoundError:
    print(f"❌ 错误：未找到昨天({yesterday})的L3预测报告")
    exit(1)
except Exception as e:
    print(f"❌ 读取预测报告失败：{str(e)}")
    exit(1)
...
accuracy_rate = accuracy_stats["correct"] / accuracy_stats["total"] * 100
