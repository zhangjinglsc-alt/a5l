...
# 确保目录存在
os.makedirs("data/review_layers", exist_ok=True)
...
try:
    ...
    if not predictions:
        print(f"⚠️ 昨天({yesterday})没有找到预测数据，任务结束")
        sys.exit(0)

    # 第二步：获取今天的实际数据（模拟/真实接口）
    # 注意：如果get_data_layer不可用，这里使用模拟数据演示，实际环境替换为真实接口
    try:
        ...
        # 模拟数据（实际使用时删除这段）
        today_actual = {
            "shanghai_index": {"change": 1.23},
            "shenzhen_index": {"change": 1.85},
            "cyb_index": {"change": 2.15},
            "main_sector": "半导体",
            "sentiment_score": 78
        }
        print("ℹ️ 使用模拟数据进行演示，实际环境请配置tools.data_layer")
...
"""
    if "langzhu" in verification_results:
        report += f"""
📈 浪主指数
- 预测：{verification_results['langzhu']['prediction']}
- 实际：上证指数{verification_results['langzhu']['actual']:+.2f}%
- 结果：{verification_results['langzhu']['result']}
"""
    if "buffett" in verification_results:
        report += f"""
🧠 巴菲特情绪
- 预测：分歧度{verification_results['buffett']['prediction']}
- 实际：情绪得分{verification_results['buffett']['actual']:.0f}
- 结果：{verification_results['buffett']['result']}
"""
    if "factor" in verification_results:
        report += f"""
📊 因子投资
- 预测：{verification_results['factor']['prediction']}风格
- 实际：主线板块{verification_results['factor']['actual']}
...
"""
    ...
    with open(f"data/skill_verification_{today}.json", "w", encoding="utf-8") as f:
        json.dump(verification_record, f, indent=2, ensure_ascii=False)

    # 输出报告
    print(report)
    print(f"\n✅ 技能验证完成，记录已保存到 data/skill_verification_{today}.json")

except FileNotFoundError:
    print(f"⚠️ 未找到昨天({yesterday})的L3预测文件 data/review_layers/layer3_comprehensive_{yesterday}.json，跳过验证")
